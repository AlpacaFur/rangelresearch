...
const TeamMember: React.FC<TeamMemberProps> = ({ member } ) => {
    return (
        <div className="flex items-center mb-8 bg-white p-4 flex-wrap gap-5">
            <img src={`/photos/${member.photo}`} alt={member.name} className="mr-8 bg-white rounded"/>
            <div>
                ...
                <p className="text-gray-600 bg-white text-md max-w-prose">{member.bio}</p>
            </div>
        </div>
    );
};
